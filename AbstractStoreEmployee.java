...
public abstract class AbstractStoreEmployee implements Employee, Store {
    ...
    public double calculateTotalEmployeePay(ArrayList<Manager> managersList, ArrayList<SalesAssociate> salesList,
        ...
        for (Manager manager : managersList) {
            totalEmployeePay += manager.calculatePay();
        }

        for (SalesAssociate sales : salesList) {
            totalEmployeePay += sales.calculatePay();
        }

        for (InventoryControl inventoryControl : inventoryControlsList) {
            totalEmployeePay += inventoryControl.calculatePay();
        }
    ...
    /**
     * Checks if the employee should be terminated (fired)
     * 
     * @return - This method returns the eligibility status for termination for an
     *         employee.
     */
    @Override
    public abstract boolean checkTerminationEligiblity();

    /**
     * Subprogram to give permission for employee to leave
     */
    public abstract void leavePermission();
